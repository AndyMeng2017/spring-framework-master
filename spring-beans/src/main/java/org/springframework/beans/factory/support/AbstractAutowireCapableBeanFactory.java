...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		if (earlySingletonExposure) {
			...
			//getEarlyBeanReference方法中，对bean再一次依赖引用，主要应用SmartInstantiationAwareBeanPostProcessor
			//其中我们熟知的AOP就是在这里将adivce动态织入bean中，若没有则直接返回bean，不做任何处理
		...
		try {
			// <5> 对bean进行填充，将各个属性值注入，其中可能存在依赖于其他bean的属性，则会递归初始化bean
		...
		if (earlySingletonExposure) {
			...
			if (earlySingletonReference != null) {
				...
				if (exposedObject == bean) {
					...
					//因为bean创建后其所依赖的bean一定是已经创建的，actualDependentBeans不为空，则表示当前bean创建后其依赖的bean却没有
					//全部创建完，也就是存在循环依赖
